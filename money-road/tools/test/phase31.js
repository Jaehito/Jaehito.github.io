...
(async()=>{
 ...
 want('단타 탭', ko.탭[0], '단타');
 want('상점 탭', ko.탭[1], '상점');
 /* 「평가 계좌」 스토리로 돌아오면서 해금 → 권한이 됐다. 회사가 열어주는 것이라
    "해금"은 게임 용어였고 "권한"은 그 세계의 말이다. */
 want('권한 탭', ko.탭[2], '권한');
 want('퀵 최대', ko.퀵최대, '최대');
 want('레버리지 3배', ko.레버리지, '3배 청산 0.67x');
 want('연승 없음', ko.스트립, '연승 없음');
 want('목표 칩', ko.목표칩, '50만원·D-13');   // 한국어 표기는 만/억으로 끊는다
 /* 시작 버튼이 결재 도장이 됐다 — 「…로 시작」이라는 문장 대신 「승 인」 아래
    금액이 한 줄. 주문표를 읽고 도장을 찍는 순서라 버튼이 동사가 아니라 결재란이다. */
 if(!/^승 인/.test(ko.시작버튼)||!/5만원 × 3배/.test(ko.시작버튼)) fail.push('시작 버튼: '+ko.시작버튼);
