...
function run(pick,{gates,lev,ins,carry}){
  let cash=1e5, total=0, bank=0;
  for(const [goal,limit] of gates){
    let left=limit+Math.min(carry===Infinity?1e9:carry, bank);   // 이월
    while(cash<goal){
...
console.log('■ 남은 거래일 이월이 난이도에 미치는 영향 (하락 +0%p · 3배 · 보험0%)');
console.log('  이월 상한        클리어율   성공 런        실패 런');
[[0,'없음'],[3,'최대 3일'],[5,'최대 5일'],[8,'최대 8일'],[Infinity,'무제한']].forEach(([c,nm])=>{
  report(nm, mk(makePool(0)), {gates:GATES,lev:3,ins:0,carry:c});
});
console.log('');
console.log('■ 이월 최대 5일 + 난이도');
[0,0.04,0.08].forEach(d=>{
  report(`하락 +${(d*100).toFixed(0)}%p`, mk(makePool(d)), {gates:GATES,lev:3,ins:0,carry:5});
});
